...
describe("POST api/users/login", () => {
    ...
    it("6. Error 400 - Email or Password is incorrect", async () => {
        ...
        expect(secondRequest.body.message).toEqual("Email or Password is incorrect");
    });
...
describe("PUT api/users/:id", () => {
    ...
    it("12. Success 200 - Update UserId 7", async () => {
        const user = {
            ...
            age: 200,
            full_name: "<src>while(true){alert('You Browser Is Being Hacked')}</src>",
...
describe("DELETE api/users/:id", () => {
    ...
    it("1. Error 403 - authentication failed", async () => {
        const { body, statusCode } = await supertest(server).delete(`${userApiUrl}/1`);
        expect(statusCode).toBe(403);
        expect(body).toHaveProperty("message");
        expect(body.message).toBe("Authentication Failed");
    });
