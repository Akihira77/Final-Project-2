...
describe("PUT /api/comments/:id", () => {
    ...
    it("1. Error 403 - Authentication Failed", async () => {
        const { statusCode, body } = await supertest(server).put(`${commentApiUrl}/1`);
        expect(statusCode).toBe(403);
        expect(body).toHaveProperty("message", "Authentication Failed");
    });
...
describe("DELETE /api/comments/:id", () => {
    ...
    it("1. Error 403 - authentication failed", async () => {
        const { body, statusCode } = await supertest(server).delete(`${commentApiUrl}/1`);
        expect(statusCode).toBe(403);
...
//# sourceMappingURL=comment.api.test.js.map